...
import cv2
                            #####ERROSION#####
def errosion(kernel, thres_img):
    img = cv2.copyMakeBorder(thresh_img, 2, 2, 2, 2, cv2.BORDER_CONSTANT) # Add padding
...
def opening(kernel, thres_img):
    ans1 = errosion(kernel, thres_img)
    ans2 = dialation(kernel, ans1)
    return ans2
